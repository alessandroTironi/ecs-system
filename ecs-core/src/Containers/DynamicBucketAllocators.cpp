...
dynamic_bucket_t::dynamic_bucket_t(size_t inBlockSize, size_t inBlockCount)
    : blockSize{inBlockSize}, blockCount{inBlockCount}
{
	allocate_bucket_instance();
}

dynamic_bucket_t::~dynamic_bucket_t()
{
	for (bucket_instance_t& bucketInstance : m_data)
	{
...
void dynamic_bucket_t::allocate_bucket_instance()
{
    bucket_instance_t instance;
...
void* dynamic_bucket_t::allocate(size_t bytes) noexcept 
{
	...
	bucket_instance_t& bucketInstance = m_data[bucketInstanceIndex];
	const size_t localIndex = index - (bucketInstanceIndex * blockCount);
    return bucketInstance.data + (localIndex * blockSize);
}

void dynamic_bucket_t::deallocate(void* ptr, size_t bytes) noexcept 
{
...
bool dynamic_bucket_t::belongs_to_this(void* ptr) const noexcept 
{
    for (const bucket_instance_t& bucketInstance : m_data)
    {
        const std::byte* p = static_cast<std::byte*>(ptr);
...
size_t dynamic_bucket_t::find_contiguous_blocks(size_t n) const noexcept 
{
...
bool dynamic_bucket_t::is_block_in_use(size_t index) const noexcept 
{
...
void dynamic_bucket_t::set_blocks_in_use(size_t index, size_t n) noexcept
{
    const size_t instanceIndex = calculate_bucket_instance_index(index);
    const size_t localIndex = index - (instanceIndex * blockCount);
...
void dynamic_bucket_t::set_blocks_free(size_t index, size_t n) noexcept
{
    const size_t instanceIndex = calculate_bucket_instance_index(index);
    const size_t localIndex = index - (instanceIndex * blockCount);
...
size_t dynamic_bucket_t::calculate_bucket_instance_index(const size_t blockIndex) const
{
    return blockIndex / blockCount;
}

size_t dynamic_bucket_t::find_bucket_instance_index(void* ptr) const
{
    const std::byte* p = static_cast<std::byte*>(ptr);
    for (size_t index = 0; index < m_data.size(); ++index)
    {
        const bucket_instance_t& instance = m_data[index];
